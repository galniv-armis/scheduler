...
const IMMEDIATE_JOB: ScheduledJob = {
...
const JOB_TO_RUN_EVERY_SECOND: ScheduledJob = {
...
describe('Test suite for scheduledJobs.service.', () => {
    ...
    describe("Test the service's setup functionality.", () => {
        ...
        beforeEach(() => {
            when(mockedFindCursor.toArray()).thenReturn(new Promise((resolve) => resolve([JOB_TO_RUN_EVERY_SECOND])))
        ...
        it('Should retrieve all non-immediate scheduled jobs from the data provider.', async () => {
            ...
            expect(scheduledJobsService._scheduledJobs.has(JOB_TO_RUN_EVERY_SECOND._id.toString())).toBe(true)
        })
        ...
        it('Should run non-immediate scheduled jobs.', async () => {
            ...
            expect(console.log).toHaveBeenCalledWith(`Making coffee with config ${JSON.stringify(JOB_TO_RUN_EVERY_SECOND.jobParams)}`);
        })
    })
    ...
    describe("Tests that start the service with empty setup.", () => {
        ...
        describe("Test the service's create functionality.", () => {
            it('Should run immediate scheduled jobs immediately & not keep CronJobs for them.', async () => {
                await scheduledJobsService.dispatchImmediate(IMMEDIATE_JOB)
                expect(console.log).toHaveBeenCalledWith(`Sending email with config  ${JSON.stringify(IMMEDIATE_JOB.jobParams)}`);
            })

            it('Should run non-immediate scheduled jobs.', async () => {
                await scheduledJobsService.create(JOB_TO_RUN_EVERY_SECOND)
        ...
        describe("Test the service's delete functionality.", () => {
            it('Should delete scheduled jobs from the CronJobs.', async () => {
                ...
                await scheduledJobsService.delete(JOB_TO_RUN_EVERY_SECOND._id)
                expect(scheduledJobsService._scheduledJobs.has(JOB_TO_RUN_EVERY_SECOND._id.toString())).toBe(false)
